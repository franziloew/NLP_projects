---
title: "Using Text Mining To Predict Prices"
output:  
  html_document:
    theme: "lumen"
    highlight: "tango"
...
```{r}
df.ger <- df %>% filter(language == "german")
df.eng <- df %>% filter(language=="english")
```
...
## Wordclouds
```{r}
corp.ger <- corpus(df$text_cleaned)
docvars(corp.ger)<-df$city   #attaching the class labels to the corpus message text
...
```{r}
df.reg1 <- df %>%
  select(city, price, overall_satisfaction,
...
```{r}
df.reg1$predict <- predict(lm.base)
...
df.reg1 %>%
  select(log_price, predict, residuals) %>%
  sample_n(10) %>%
  knitr::kable(align = "l")
```

```{r}
ggplot(df.reg1, aes(x = accommodates, y=log_price)) +
  geom_point() +
  geom_point(aes(y=predict), color = "blue", alpha = 0.5)
```
